<template>
    <div class="p-6">
        <h1 class="text-2xl font-bold mb-4">Customer 管理</h1>
        <div class="mb-4">
            <button @click="showForm = true" class="bg-blue-500 text-white px-4 py-2 rounded">新增 Customer</button>
        </div>
        <div v-if="showForm" class="mb-4 p-4 bg-gray-100 rounded">
            <h2 class="text-lg font-semibold">新增 Customer</h2>
            <form @submit.prevent="submitForm">
                <div class="mb-2">
                    <label class="block">名稱/主題</label>
                    <input v-model="form.name" type="text" class="w-full border rounded px-2 py-1" required>
                </div>
                <div class="mb-2" v-if="['product', 'marketing'].includes('customer')">
                    <label class="block">描述</label>
                    <textarea v-model="form.description" class="w-full border rounded px-2 py-1"></textarea>
                </div>
                <div class="mb-2" v-if="'product' === 'customer'">
                ...
                <div class="mb-2" v-if="'order' === 'customer'">
        ...
        <div class="bg-white p-4 rounded shadow">
            <h2 class="text-lg font-semibold mb-2">Customer 列表</h2>
            ...
            <div v-else>
                ...
                <div v-if="!items.length" class="text-gray-500">無 Customer 數據</div>
            </div>
        </div>
    </div>
</template>
<script setup>
import { ref } from 'vue';
import { fetchCustomerList, createCustomer, deleteCustomer } from '@/api/api';
const items = ref([]);
...
async function submitForm() {
    const response = await createCustomer(form.value);
    if (response.success) {
